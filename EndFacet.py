...
class Slab():
  ...
  def Nu(self,m,p):
    ...
    return A[m] * Bt(p) * cos(K[m]*d) * \
      ((K[m]*tan(K[m]*d)*cos(p*d) - p*sin(p*d)) / eps/(K[m]**2 - p**2) - \
      (p*sin(p*d) - g[m]*cos(p*d)) / (g[m]**2 + p**2) )
  ...
  def Kappa(self,m,p):
    ...
    return A[m] * Bt(p) * cos(K[m]*d) * \
      ((K[m]*tan(K[m]*d)*cos(p*d) - p*sin(p*d)) / (K[m]**2 - p**2) - \
      (p*sin(p*d) - g[m]*cos(p*d)) / (g[m]**2 + p**2) )
  ...
  def Zeta(self,p1,p2):
    ...
    return - k**2*(eps-1)*Bt(p2)*Br(p1)/2* (sin((o(p1)+p2) *d)/(o(p1)+p2) + sin((o(p1)-p2)*d)/(o(p1)-p2)) \
    + Br(p1)*Bt(p2)*sin(o(p1)*d)*cos(p2*d) * o(p1) *(eps -1)/eps 
  ...
  def f(self,p1,p2):
    ...
    return - k**2*(eps-1)*Bt(p2)*Br(p1)/2/eps* (sin((o(p1)+p2) *d)/(o(p1)+p2) + sin((o(p1)-p2)*d)/(o(p1)-p2)) \
    + Br(p1)*Bt(p2)*cos(o(p1)*d)*sin(p2*d) * p2 *(eps -1)/eps 
